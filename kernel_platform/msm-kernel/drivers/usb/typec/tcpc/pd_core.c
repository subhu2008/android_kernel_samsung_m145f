...
static void pd_parse_log_src_cap_ext(struct pd_source_cap_ext *cap)
{
	...
	pr_info("%s touch_temp = %d, source_inputs = 0x%x, batteries = 0x%x, source_pdp = 0x%x, epr_source_pdp = 0x%x\n",
		__func__,
		cap->touch_temp,
		cap->source_inputs,
		cap->batteries,
...
{
#ifdef CONFIG_USB_PD_REV30_SYNC_SVDM_VER
		...
		uint8_t msg, bool ext, uint16_t count, const uint32_t *data)
{
	...
	if (!rv && SC2150A_DID == chip_id &&
		...
		pd_enable_timer(pd_port, PD_TIMER_NO_SEND);
	}
	return ret;
}
