...
#define PROBE_CNT_MAX			1000
/* 10ms * 100 = 1000ms = 1s */
#define USB_TYPE_POLLING_INTERVAL	10
#define USB_TYPE_POLLING_CNT_MAX	300

extern void tcpc_upm6918_enter_hiz_mode(void);
extern void tcpc_upm6918_exit_hiz_mode(void);
extern void tcpc_upm6918_set_chargecurrent(void);
int hiz_flag = 0;
...
static void usb_dwork_handler(struct work_struct *work)
{
	...
	case DR_DEVICE:
		rpmd->bbc_psy = power_supply_get_by_name("bbc");
			if (!rpmd->bbc_psy) {
				pr_err("%s get bbc psy fail\n", __func__);
			}
		ret = power_supply_get_property(rpmd->bbc_psy, POWER_SUPPLY_PROP_CHARGE_TYPE, &propval);
		val = propval.intval;

		dev_info(rpmd->dev, "%s polling_cnt = %d, ret = %d type = %d\n",
				    __func__, ++rpmd->usb_type_polling_cnt, ret, val);
		if (ret < 0 || val == POWER_SUPPLY_USB_TYPE_UNKNOWN) {
			...
			break;
		} else if (val != POWER_SUPPLY_USB_TYPE_SDP &&
				val != POWER_SUPPLY_USB_TYPE_CDP &&
				val != POWER_SUPPLY_USB_TYPE_PD &&
				val != POWER_SUPPLY_USB_TYPE_FLOAT)
			break;
				...
				unsigned long event, void *data)
{
	...
	dev_err(rpmd->dev, "lsw %s %d event 0x%lx!!\n", __func__, __LINE__, event);
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if((rpmd->sink_mv_new == 5000) && (rpmd->sink_ma_new == 100))
		{
			tcpc_upm6918_set_chargecurrent();
			dev_err(rpmd->dev, "%s TCPC 5V100MA\n", __func__);
		}

		if((rpmd->sink_mv_new == 5000) && (rpmd->sink_ma_new <= 0))
		{
			if(noti->vbus_state.type == TCP_VBUS_CTRL_PD_REQUEST)
			{
				/* disable VBUS power path */
				//tcpc_upm6918_enter_hiz_mode();
				hiz_flag = 1;
				dev_err(rpmd->dev, "%s %dmv and %dma hiz !!\n", __func__, rpmd->sink_mv_new, rpmd->sink_ma_new);
			}
			
		}
		else if(hiz_flag == 1)
		{
			if(noti->vbus_state.type != TCP_VBUS_CTRL_PD_STANDBY_DOWN)
			{
				tcpc_upm6918_exit_hiz_mode();
				hiz_flag = 0;
				dev_err(rpmd->dev, "%s tcpc exit hiz!!\n", __func__);
			}
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (new_state != TYPEC_UNATTACHED) {
			...
			//pr_err("%s USB is online, cc_or = %d\n",__func__, val.intval);
		} else {
			val.intval = 0;
			//pr_err("%s USB is offline, cc_or = %d\n",__func__, val.intval);
		}
...
//-Bug 697289 ,lishuwen.wt,ADD,20211027, add typec cc orientation node
#endif
		/* -Bug676386,liangjianfeng.wt,ADD,20210818, add sysfs node */
...
static int rt_pd_manager_probe(struct platform_device *pdev)
{
	...
	pr_err("(%s) probe_cnt = %d\n ", RT_PD_MANAGER_VERSION, ++probe_cnt);
	...
	if (!rpmd->tcpc->tcpc_complete) {
		dev_err(rpmd->dev, "%s tcpc complete fail\n", __func__);
		ret = -EPROBE_DEFER;
		if (probe_cnt >= PROBE_CNT_MAX)
			goto out;
		else
	...
	rpmd->bbc_psy = power_supply_get_by_name("bbc");
	if (!rpmd->bbc_psy) {
		dev_err(rpmd->dev, "%s get bbc psy fail\n", __func__);
                ret = -EPROBE_DEFER;
                if (probe_cnt >= PROBE_CNT_MAX)
                        goto out;
                else
                        goto err_get_bbc_psy;
        }
	...
	if (!rpmd->tcpc->tcpc_pd_probe) {
		dev_err(rpmd->dev, "%s pd logic fail!\n", __func__);
		ret = -EPROBE_DEFER;
		if (probe_cnt >= PROBE_CNT_MAX)
			goto out;
		else
	...
	pr_err("rt_pd_manager_probe successful !\n");
