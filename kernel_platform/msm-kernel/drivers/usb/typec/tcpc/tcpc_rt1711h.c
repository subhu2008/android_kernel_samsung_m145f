...
RT_REG_DECL(CPS8851_REG_PD_SDAC, 1, RT_NORMAL_WR_ONCE, {});
RT_REG_DECL(CPS8851_REG_PD_TCPC, 1, RT_NORMAL_WR_ONCE, {});
...
static const rt_register_map_t rt1711_chip_regmap[] = {
	...
	RT_REG(CPS8851_REG_PD_SDAC),
	RT_REG(CPS8851_REG_PD_TCPC),
...
static int rt1711_init_alert(struct tcpc_device *tcpc)
{
	...
	dev_info(chip->dev, "%s t_c_c name = %s, gpio = %d\n",
			    __func__, chip->tcpc_desc->name, chip->irq_gpio);
...
/*************************************************/
 	...
 	ret = devm_gpio_request_one(chip->dev, chip->irq_gpio, GPIOF_DIR_IN,
  			...
  			"rt1711_intr_gpio.%s", dev_name(chip->dev)));
	if (ret < 0) {
	...
	struct tcpc_device *tcpc, uint8_t power_role, uint8_t data_role)
{
	...
	return rt1711_i2c_write8(tcpc, TCPC_V10_REG_TX_HDR, hdr);
}
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	// 	pr_err("%s no intr_gpio info\n", __func__);
 	...
 	ret = of_get_named_gpio(np, "rt1711pd,intr_gpio_num", 0);
  	if (ret < 0) {
  		pr_err("%s no rt1711pd,intr_gpio_num(%d)\n",
  				      __func__, ret);
  	}else
		chip->irq_gpio = ret;
...
static inline int rt1711h_check_revision(struct i2c_client *client)
{
	...
	if ((pid != RICHTEK_1711_PID) && (pid != SC2150A_PID) &&
		(pid != SC6601_PID) && (pid != CPS_8851_PID)) {
				...
				const struct i2c_device_id *id)
{
	...
	pr_err("%s probe OK![%d]\n", __func__,chip->tcpc->tcpc_pd_probe);
	return 0;
