...
extern void x_write_cp_disable_2130(void);

#include <linux/i2c.h>
#include <linux/module.h>
#include <linux/init.h>

int i2c_write_input_limit_volt_6918(void) {
    struct i2c_adapter *adapter;
    struct i2c_msg msg;
    int ret;
    u8 buf[2] = {0x06, 0x2F};
	PE_INFO("find i2c_write_input_limit_volt_6918\n");
    adapter = i2c_get_adapter(2); // I2C1
    if (!adapter) {
        PE_INFO("i2c_write_input_limit_volt_6918 Failed to get I2C adapter\n");
        return -ENODEV;
    }
    msg.addr = 0x6B;
    msg.flags = 0;
    ...
    if (ret != 1) {
		PE_INFO("i2c_write_input_limit_volt_6918 I2C transfer failed: %d\n", ret);
		i2c_put_adapter(adapter);
    	return -EIO;
    }
    i2c_put_adapter(adapter);
    PE_INFO("i2c_write_input_limit_volt_6918 I2C transfer successful\n");
    return 0;
}
EXPORT_SYMBOL(i2c_write_input_limit_volt_6918);

/*
static int i2c_write_cp_disabled_2130(void) {
    struct i2c_adapter *adapter;
    struct i2c_msg msg;
    int ret;
    u8 buf[2] = {0x06, 0x2F};
        mutex_lock(&i2c_mutex);
        PE_INFO("find i2c_write_cp_disabled_2130\n");
    adapter = i2c_get_adapter(2); // I2C1
    if (!adapter) {
        mutex_unlock(&i2c_mutex);
        PE_INFO("i2c_write_cp_disabled_2130 Failed to get I2C adapter\n");
        return -ENODEV;
    }
    msg.addr = 0x6D;
    msg.flags = 0;
...
void pe_snk_select_capability_entry(struct pd_port *pd_port)
{
	...
	{
		x_write_cp_disable_2130();
		i2c_write_input_limit_volt_6918();
		PE_INFO("pe_snk_select_capability_entry, %d  %d\n", pd_port->request_v_new, pd_port->request_i_new);
	}
	*/
...
#ifdef CONFIG_USB_PD_REV30_SNK_CAP_EXT_LOCAL
