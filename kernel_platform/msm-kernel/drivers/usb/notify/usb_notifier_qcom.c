...
#if IS_ENABLED(CONFIG_USB_NOTIFY_EXTCON)
static const unsigned int usb_notifier_extcon_cable[] = {
	EXTCON_USB,
	EXTCON_USB_HOST,
	EXTCON_NONE,
};

static int extcon_init(struct usb_notifier_platform_data *pdata)
{
	int ret = 0;
	 ...
	 */
	pdata->extcon = devm_extcon_dev_allocate(pdata->dev, usb_notifier_extcon_cable);
	if (IS_ERR(pdata->extcon)) {
		ret = PTR_ERR(pdata->extcon);
		pr_err("%s extcon dev alloc fail(%d)\n",
				   __func__, ret);
		goto out;
	}

	ret = devm_extcon_dev_register(pdata->dev, pdata->extcon);
	if (ret) {
	...
	extcon_set_property_capability(pdata->extcon, EXTCON_USB,
				       EXTCON_PROP_USB_TYPEC_POLARITY);
	extcon_set_property_capability(pdata->extcon, EXTCON_USB,
				       EXTCON_PROP_USB_SS);
	extcon_set_property_capability(pdata->extcon, EXTCON_USB_HOST,
				       EXTCON_PROP_USB_TYPEC_POLARITY);
	extcon_set_property_capability(pdata->extcon, EXTCON_USB_HOST,
				       EXTCON_PROP_USB_SS);
out:
...
static int qcom_set_host(bool enable)
{
	...
	if (enable) {
		//val.intval = tcpm_inquire_cc_polarity(pdata->tcpc);
		val.intval = 0;
		extcon_set_property(pdata->extcon, EXTCON_USB_HOST,
		...
		extcon_set_state_sync(pdata->extcon, EXTCON_USB_HOST, true);
	} else
		extcon_set_state_sync(pdata->extcon, EXTCON_USB_HOST, false);
#else
...
static int qcom_set_peripheral(bool enable)
{
	...
	if (enable) {
		//val.intval = tcpm_inquire_cc_polarity(pdata->tcpc);
		val.intval = 0;
		extcon_set_property(pdata->extcon, EXTCON_USB,
					EXTCON_PROP_USB_TYPEC_POLARITY, val);

		val.intval = 1;
		extcon_set_property(pdata->extcon, EXTCON_USB, EXTCON_PROP_USB_SS, val);
		extcon_set_state_sync(pdata->extcon, EXTCON_USB, true);
	} else
		extcon_set_state_sync(pdata->extcon, EXTCON_USB, false);
#else
