...
int upm6918_set_input_volt_limit(struct upm6918 *upm, int volt)
{
	...
	ret = upm6918_update_bits(upm, UPM6918_REG_06, REG06_VINDPM_MASK,
				   val << REG06_VINDPM_SHIFT);
	pr_err("upm6918_set_input_volt_limit volt =%d\n",volt);
	return ret;
}
...
static int upm6918_init_device(struct upm6918 *upm)
{
	...
	//ret = upm6918_set_input_volt_limit(upm, 4600);
	if (ret)
